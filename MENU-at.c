...
void menu_principal() {
    ...
    while(1) {
        ...
        switch(opcao) {
            case 1: menu_estoque(); break;
            case 2: menu_financeiro(); break;
...
void menu_estoque() {
    ...
    while(1) {
        printf("\n=== CONTROLE DE ESTOQUE ===\n");
        printf("1. Produto A (50 unid)\n");
        printf("2. Produto B (30 unid)\n");
        printf("3. Produto C (20 unid)\n");
        printf("4. Produto D (15 unid)\n");
        printf("5. Voltar\n");
        printf("Escolha: ");
        scanf("%d", &opcao);

        switch(opcao) {
            case 1: printf("50 unidades disponiveis\n"); break;
            case 2: printf("30 unidades disponiveis\n"); break;
            case 3: printf("20 unidades disponiveis\n"); break;
            case 4: printf("15 unidades disponiveis\n"); break;
            case 5: return;
...
float processar_pagamento(float total) {
    ...
    while(1) {
        ...
        if(opcao == 1) {
            float desconto = 0;
            if(total <= 50) desconto = total * 0.05;
            else if(total < 100) desconto = total * 0.10;
            else desconto = total * 0.18;
    ...
    while(1) {
        ...
        switch(opcao) {
            ...
            case 5:
                if(total_compra > 0) {
                    printf("\n=== RESUMO ===\n");
                    if(total_categoria_A > 0) printf("Produto A: R$%.2f\n", total_categoria_A);
                    if(total_categoria_B > 0) printf("Produto B: R$%.2f\n", total_categoria_B);
                    if(total_categoria_C > 0) printf("Produto C: R$%.2f\n", total_categoria_C);
                    if(total_categoria_D > 0) printf("Produto D: R$%.2f\n", total_categoria_D);
                    printf("TOTAL: R$%.2f\n", total_compra);
